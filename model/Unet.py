...
class dynamic_double_conv(nn.Module):
    def __init__(self, in_channel, out_channel):
        super().__init__()

        self.dconv = nn.Sequential(
            DyConv(4, in_channel, out_channel, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(out_channel),
            nn.ReLU(),
            DyConv(4, out_channel, out_channel, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(out_channel),
            nn.ReLU(),
        )
    def forward(self, x):
...
class Unet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.interpolate(x_mid, scale_factor=2, mode="bilinear", align_corners=True)
